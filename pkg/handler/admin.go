...
func (h *Handler) adminChangeLuckUser(c *gin.Context) {
	type Body struct {
		TeleId   int64  `json:"tele_id"`
	...
	statusCode, err := h.services.AdminChangeLuckUser(body.TeleId, body.NeedLuck)
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
	...
	c.JSON(statusCode, map[string]interface{}{
		"status":  statusCode,
		"message": "успешное изменение удачи пользователя",
	})
}
