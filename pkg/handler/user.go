...
func (h *Handler) checkIsCaptcha(c *gin.Context) {
	type Body struct {
	...
	res, statusCode, err := h.services.CheckIsCaptcha(body.ValueUser)
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
	...
	if res {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": "капча существует",
			"result":  res,
		})
	} else {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": "капча не существует",
			"result":  res,
		})
	}
}
...
func (h *Handler) updateTradingAsset(c *gin.Context) {
	type Body struct {
		TeleId      int64   `json:"tele_id"`
	...
	statusCode, err := h.services.UpdateTradingAsset(appl_row.UserUpdateTradingAsset(body))
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
	...
	c.JSON(statusCode, map[string]interface{}{
		"status":  statusCode,
		"message": "успешная обновление данных трейдинга пользователя",
	})
}

func (h *Handler) updateTradingInvestPrice(c *gin.Context) {
	type Body struct {
		TeleId          int64   `json:"tele_id"`
	...
	statusCode, err := h.services.UpdateTradingInvestPrice(appl_row.UserUpdateTradingInvestPrice(body))
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
...
func (h *Handler) updateTradingWaitTime(c *gin.Context) {
	type Body struct {
		TeleId         int64 `json:"tele_id"`
	...
	statusCode, err := h.services.UpdateTradingWaitTime(appl_row.UserUpdateTradingWaitTime(body))
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
...
func (h *Handler) updateTradingMovePrice(c *gin.Context) {
	type Body struct {
		TeleId        int64  `json:"tele_id"`
	...
	statusCode, err := h.services.UpdateTradingMovePrice(appl_row.UserUpdateTradingMovePrice(body))
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
...
func (h *Handler) getUserTradingParams(c *gin.Context) {
	type Body struct {
		TeleId int64 `json:"tele_id"`
	}
	var body Body
	...
	res, statusCode, err := h.services.GetUserTradingParams(body.TeleId)
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
	...
	if len(res) > 0 {
		c.JSON(http.StatusOK, map[string]interface{}{
			"status":  http.StatusOK,
			"message": "успешное получение данных трейдинга пользователя",
			"result":  res,
		})
	} else {
		c.JSON(http.StatusOK, map[string]interface{}{
...
func (h *Handler) getUserTotalEarn(c *gin.Context) {
	type Body struct {
		TeleId int64 `json:"tele_id"`
	}
	var body Body
	...
	res, statusCode, err := h.services.GetUserTotalEarn(body.TeleId)
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
	...
	if len(res) > 0 {
		c.JSON(http.StatusOK, map[string]interface{}{
			"status":  http.StatusOK,
			"message": "успешное получение всего заработка пользователя",
			"result":  res,
		})
	} else {
		c.JSON(http.StatusOK, map[string]interface{}{
...
func (h *Handler) changeUserTotalEarn(c *gin.Context) {
	type Body struct {
		TeleId    int64   `json:"tele_id"`
	...
	statusCode, err := h.services.ChangeUserTotalEarn(body.TeleId, body.EarnMoney)
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
	...
	c.JSON(statusCode, map[string]interface{}{
		"status":  statusCode,
		"message": "успешная изменение всего заработка",
	})
}

func (h *Handler) getUserCurrentLuck(c *gin.Context) {
	type Body struct {
		TeleId int64 `json:"tele_id"`
	}
	var body Body
	...
	res, statusCode, err := h.services.GetUserCurrentLuck(body.TeleId)
	if err != nil {
		c.JSON(statusCode, map[string]interface{}{
			"status":  statusCode,
			"message": err.Error(),
	...
	if len(res) > 0 {
		c.JSON(http.StatusOK, map[string]interface{}{
			"status":  http.StatusOK,
			"message": "успешное получение текущей удачи пользователя",
			"result":  res,
		})
	} else {
		c.JSON(http.StatusOK, map[string]interface{}{
