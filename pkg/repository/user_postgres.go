...
func (r *UserPostgres) CheckIsCaptcha(valueUser int64) (bool, int, error) {
	var isCaptcha bool
	err := r.db.QueryRow("SELECT user_check_captcha($1)", valueUser).Scan(&isCaptcha)
	if err != nil {
		return false, http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_check_captcha из базы данных, %s", err)
	}
	return isCaptcha, http.StatusOK, nil
}
...
func (r *UserPostgres) UpdateTradingAsset(tradingAssetForm appl_row.UserUpdateTradingAsset) (int, error) {
	tradingAssetFormJson, _ := json.Marshal(tradingAssetForm)
	_, err := r.db.Exec("SELECT user_update_trading_asset($1)", tradingAssetFormJson)
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_update_trading_asset из базы данных, %s", err)
	}
	return http.StatusOK, nil
}

func (r *UserPostgres) UpdateTradingInvestPrice(tradingAssetForm appl_row.UserUpdateTradingInvestPrice) (int, error) {
	tradingAssetFormJson, _ := json.Marshal(tradingAssetForm)
	_, err := r.db.Exec("SELECT user_update_trading_invprice($1)", tradingAssetFormJson)
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_update_trading_invprice из базы данных, %s", err)
	}
	return http.StatusOK, nil
}

func (r *UserPostgres) UpdateTradingWaitTime(tradingAssetForm appl_row.UserUpdateTradingWaitTime) (int, error) {
	tradingAssetFormJson, _ := json.Marshal(tradingAssetForm)
	_, err := r.db.Exec("SELECT user_update_trading_wtimesec($1)", tradingAssetFormJson)
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_update_trading_wtimesec из базы данных, %s", err)
	}
	return http.StatusOK, nil
}

func (r *UserPostgres) UpdateTradingMovePrice(tradingAssetForm appl_row.UserUpdateTradingMovePrice) (int, error) {
	tradingAssetFormJson, _ := json.Marshal(tradingAssetForm)
	_, err := r.db.Exec("SELECT user_update_trading_moveprice($1)", tradingAssetFormJson)
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_update_trading_moveprice из базы данных, %s", err)
	}
	return http.StatusOK, nil
}

func (r *UserPostgres) GetUserTradingParams(teleId int64) ([]appl_row.UserGetUserTradingParams, int, error) {
	var userGetUserTradingParams []appl_row.UserGetUserTradingParams
	var userGetUserTradingParamsByte []byte
	err := r.db.QueryRow("SELECT user_get_trading_params($1)", teleId).Scan(&userGetUserTradingParamsByte)
	if err != nil {
		return []appl_row.UserGetUserTradingParams{}, http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_get_trading_params из базы данных, %s", err)
	}
	err = json.Unmarshal(userGetUserTradingParamsByte, &userGetUserTradingParams)
	if err != nil {
		return []appl_row.UserGetUserTradingParams{}, http.StatusInternalServerError, fmt.Errorf("ошибка конвертации в функции GetUserTradingParams, %s", err)
	}
	return userGetUserTradingParams, http.StatusOK, nil
}

func (r *UserPostgres) GetUserTotalEarn(teleId int64) ([]appl_row.UserGetTotalEarn, int, error) {
	var userTotalEarn []appl_row.UserGetTotalEarn
	var userTotalEarnByte []byte
	err := r.db.QueryRow("SELECT user_get_total_earn($1)", teleId).Scan(&userTotalEarnByte)
	if err != nil {
		return []appl_row.UserGetTotalEarn{}, http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_get_trading_params из базы данных, %s", err)
	}
	err = json.Unmarshal(userTotalEarnByte, &userTotalEarn)
	if err != nil {
		return []appl_row.UserGetTotalEarn{}, http.StatusInternalServerError, fmt.Errorf("ошибка конвертации в функции GetUserTotalEarn, %s", err)
	}
	return userTotalEarn, http.StatusOK, nil
}

func (r *UserPostgres) ChangeUserTotalEarn(teleId int64, earnMoney float64) (int, error) {
	_, err := r.db.Exec("SELECT user_update_total_earned($1, $2)", teleId, earnMoney)
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_update_total_earned из базы данных, %s", err)
	}
	return http.StatusOK, nil
}

func (r *UserPostgres) GetUserCurrentLuck(teleId int64) ([]appl_row.UserGetCurrentLuck, int, error) {
	var userCurrentLuck []appl_row.UserGetCurrentLuck
	var userCurrentLuckByte []byte
	err := r.db.QueryRow("SELECT user_get_curr_luck($1)", teleId).Scan(&userCurrentLuckByte)
	if err != nil {
		return []appl_row.UserGetCurrentLuck{}, http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции user_get_curr_luck из базы данных, %s", err)
	}
	err = json.Unmarshal(userCurrentLuckByte, &userCurrentLuck)
	if err != nil {
		return []appl_row.UserGetCurrentLuck{}, http.StatusInternalServerError, fmt.Errorf("ошибка конвертации в функции GetUserCurrentLuck, %s", err)
	}
	return userCurrentLuck, http.StatusOK, nil
}
