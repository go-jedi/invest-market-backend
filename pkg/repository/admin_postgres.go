...
func (r *AdminPostgres) AdminChangeLuckUser(teleId int64, needLuck string) (int, error) {
	_, err := r.db.Exec("SELECT admin_change_luck_user($1, $2)", teleId, needLuck)
	if err != nil {
		return http.StatusInternalServerError, fmt.Errorf("ошибка выполнения функции admin_change_luck_user из базы данных, %s", err)
	}
	return http.StatusOK, nil
}
